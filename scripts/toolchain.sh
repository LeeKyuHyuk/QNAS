...
step "[1/34] Create toolchain directory."
rm -rf $BUILD_DIR $TOOLS_DIR
mkdir -pv $BUILD_DIR $TOOLS_DIR
ln -svf . $TOOLS_DIR/usr

step "[2/34] Create the sysroot directory"
...
step "[14/34] Linux 5.13.13 API Headers"
...
make -j$PARALLEL_JOBS install -C $BUILD_DIR/ncurses-6.2
rm -rf $BUILD_DIR/ncurses-6.2

step "[32/34] MariaDB 10.6.4"
...
cp -v $BUILD_DIR/mariadb-10.6.4/build/extra/comp_err $TOOLS_DIR/bin/comp_err
cp -v $BUILD_DIR/mariadb-10.6.4/build/sql/gen_lex_hash $TOOLS_DIR/bin/gen_lex_hash
cp -v $BUILD_DIR/mariadb-10.6.4/build/sql/gen_lex_token $TOOLS_DIR/bin/gen_lex_token
cp -v $BUILD_DIR/mariadb-10.6.4/build/dbug/factorial $TOOLS_DIR/bin/factorial
cp -v $BUILD_DIR/mariadb-10.6.4/build/scripts/comp_sql $TOOLS_DIR/bin/comp_sql
make -j$PARALLEL_JOBS install -C $BUILD_DIR/mariadb-10.6.4/build
rm -rf $BUILD_DIR/mariadb-10.6.4

step "[33/34] mkpasswd 5.0.26"
